...
MultiPressButton::MultiPressButton(
    unsigned pin, uint8_t mode, unsigned long debounce_ms, unsigned long longPress_ms, unsigned long multi_press_window_ms)
    : pin(pin),
      mode(mode),
...
MultiPressButton::_isBufferFull() const
{
...
void ICACHE_RAM_ATTR
MultiPressButton::_pollButtons()
{
    for (MultiPressButton* button : buttonList) {
        ButtonPosition buttonPosition = digitalRead(button->getPin()) == LOW ? PRESSED : RELEASED;
...
void
MultiPressButton::_processEvent(ButtonEvent event)
{
    switch (state) {
        case IDLE:
        case WAITING_MULTIPRESS:
            if (event.buttonPosition == PRESSED) {
                BUTTON_DEBUG_PRINTLN("New press detected in IDLE or WAITING_MULTIPRESS state");
                longPressed = false;
                pressStart = event.timestamp;
                state = PRESSING;
            }

            if (state == WAITING_MULTIPRESS && ((event.timestamp - lastPressTime) > MULTI_PRESS_WINDOW)) {
                BUTTON_DEBUG_PRINTLN("Multi-press window timeout with " + String(pressCount) + " presses");
        ...
        case PRESSING:
            if (event.buttonPosition == PRESSED) {
                if (!longPressed && pressCount == 0 && (event.timestamp - pressStart >= LONG_PRESS_DURATION)) {
...
void
MultiPressButton::setActions(
    std::function<void()> single, std::function<void()> doubl, std::function<void()> triple, std::function<void()> longPress)
{
    singleAction = single;
    doubleAction = doubl;
    tripleAction = triple;
...
unsigned
MultiPressButton::getPin() const
{
    return pin;
}

void
MultiPressButton::update()
{
    while (readIndex != writeIndex) {
        ButtonEvent event = {eventBuffer[readIndex].timestamp, eventBuffer[readIndex].buttonPosition};
        BUTTON_DEBUG_PRINTF("Event: %s at %lu\n", event.buttonPosition == PRESSED ? "PRESSED" : "RELEASED", event.timestamp);
        _processEvent(event);
